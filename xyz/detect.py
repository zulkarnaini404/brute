...
def file_cp(ENG):
	...
	if len(cepe)==0:
		if "bahasa_inggris" in ENG:
			exit("%s╰─ %sfile does not exist "%(p,m))
		else:
			exit("%s╰─ %sfile tidak tersedia "%(p,m))
	else:
		if "bahasa_inggris" in ENG:
			tulis(Panel(f"{Te}{U} •{O} Account results{K} CP{O} stored in your folder ",style='#FF0022'))
		else:
		...
		if "bahasa_inggris" in ENG:
			tulis(Panel(f"{Te}{U} •{O} Please select the number you want to check ",style='#FF0022'))
		else:
			tulis(Panel(f"{Te}{U} •{O} Silahkan pilih nomor yang ingin di cek ",style='#FF0022'))
		if "bahasa_inggris" in ENG:
			file = input(f"{p}╰─{o} number {m}>{k} ")
		else:
...
def opsi(ENG,hasil):
	if "bahasa_inggris" in ENG:
		try:
			...
			exit("%s╰─%s filename %s is not available"%(p,m,romi))
		tulis(Panel(f"{Te} {U}•{O} Airplane mode 5 seconds first ",style='#FF0022'))
		pw=input("%s╰─%s change password on one tab account? y/t %s> %s"%(p,o,m,k))
		if pw in['y','Y']:
			...
			if len(pw2) <= 5:
				...
				pwbaru.append(pw2)
		tulis(Panel(f"{Te}{U} •{O} total account {M}: {K}{str(len(file_cp))}",style='#FF0022'))
		nomor = 0
		for fb in file_cp:
			akun = fb.replace("\n","")
			ngecek  = akun.split(" ◊ ")
...
def mengecek(user,pw,ENG):
	...
	if "c_user" in session.cookies.get_dict():
		if "Akun Anda Dikunci" in urlPost.text:
			if "bahasa_inggris" in ENG:
				print("\r%s╰─%s new session locked account"%(p,m))
			else:
				print("\r%s╰─%s akun terkunci sesi new"%(p,m))
		else:
			if "bahasa_inggris" in ENG:
				print("\r%s╰─%s the account does not checkpoint, please login "%(p,h))
			else:
		...
		number=0
		if "bahasa_inggris" in ENG:
			print("\r%s╰──%s there is %s%s%s option %s:"%(p,o,p,str(len(cek)),o,m));jeda(0.07)
		else:
		...
		if "bahasa_inggris" in ENG:
			print("%s╰─%s login failed, please check your id and password again"%(p,m))
		else:
